...
public class Controller extends Kaestchen{
	
	//set window size:
	static int pxlsize = 10; static int wndwsize = 100;
	
	//variables:
	int startPopCount = 100;
	int wayLength = 200;
	double mutProbability = 0.03;				
	double crossoverPercent = 0.99;			
	...
	public void tasteClick(String taste) {
		...
		if((taste.equals("S") && ready) || startSet == false) { 
			g = new Genetic(startPopCount, goalX, goalY, startX, startY, wayLength, mutProbability, crossoverPercent);
			g.initialize();
			running = true;
			startSet = true;
			goalSet = true;
			t.schedule(new TimerTask() {
                ...
                public void run() {
                    ...
                    drawFittest();
                    System.out.println("Fitness vom Besten: " + g.fittest.fitness + "\n" + "akt. Generation: " + genAct);
                }
            }, 0, genDuration);
		}
	...
	public void mausLeftClick(int x, int y) {
		if(startSet == false) {
			farbeSetzen(startX, startY, "weiﬂ");
			startX = x; startY = y;
			startSet = true;
			farbeSetzen(startX, startY, "blau");
		}
	...
	public void drawRndm() {
		for(int i = 0; i < wndwsize * 2; i++) {
			for(int j = 0; j < wndwsize; j++) {
				if(Math.random() < rndmWallProb) walls.add(new Wall(i, j));
			}
		} 
	}

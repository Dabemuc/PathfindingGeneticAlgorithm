...
public class Genetic{
	...
	public Genetic(int startPopCount, int goalX, int goalY, int startX, int startY, int wayLength, double mutProbability, double crossoverPercent) {
		this.goalX = goalX;
		this.goalY = goalY;
	...
	public void crossover() {
		for(int i = 0; i < startPopCount; i++) {
			...
			for(int j = 0; j <  indv.get(i).getWay().size() * crossoverPercent; j++) {
	...
	public void getFittest() {
		...
		for(int i = 0; i < startPopCount; i++) {
			if(indv.get(i).getFitness() < fittest.fitness) {
				fittest = indv.get(i);
				//System.out.println("akt. Fittest: " + i);
			}
		}
	}
